(function(root, factory) {
 ...
 'use strict';
    ...
    if (typeof schema['default'] !== 'undefined') {
      ...
      for (var key in schema.properties) {
        if (schema.properties.hasOwnProperty(key)) {
          ...
          if (typeof schema.properties[key] === 'undefined' || (schema.required != null && !schema.required.includes(key))) {
  ...
  return function (schema, definitions, options) {
    // just don't pass in a single definition named "scaffold"...
    if (definitions && definitions.scaffold && Object.keys(definitions).length === 1 && arguments.length === 2) {
      options = { 
        scaffold: definitions.scaffold
      };
